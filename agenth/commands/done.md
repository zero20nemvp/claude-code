...
Read:
- `$DIR/state.json` - get humanTask details and check aiTasks for completions
- `$DIR/velocity.json` - get current velocity data
- `$DIR/goals.json` - to update milestone progress
...
2. If any AI tasks completed since last check:
   ...
   ```
3. Update milestone progress for AI-completed tasks
4. Archive completed AI tasks (move to history or remove)

## STEP 4: Milestone Progress Update (Human Task)

Ask the human: "Which milestone(s) did this task advance?"
...
- For each selected milestone:
  ...
  - Update milestone.progress in goals.json
...
- Update goal's current_state to reflect new reality
- This informs future task generation

## STEP 6: Velocity Tracking

Update `velocity.json`:
- Add to history: {task description, points, estimatedBlocks, actualBlocks, completedAt, milestones}
...
1. **Analyze all incomplete milestones** for new tasks
...
New AI work started:
→ [AI task 3] (just dispatched)
→ [AI task 4] (just dispatched)

Combined milestone progress:
- goal-xxx → Milestone N: [name] now at X% (+Y%)
...
```
⚠️ URGENT blocker detected: [description]
...
**Continuous Work:**
- After recording completion, immediately analyze for new work
- Dispatch AI agents if tasks available
- Surface human blocker if identified
- **Never leave the system idle**

**Parallel Progress:**
- Human completes 1 task
- But AI may have completed multiple tasks in parallel
- Show combined impact on milestones

**State Transitions:**
- From: Human working + AI working
- To: Human idle + AI working (State A)
- Or: New human task needed (State B)

**Focus on Milestones:**
- No pre-planned tasks to mark complete
- Only milestones to advance
- Milestone progress is the measure of completion
